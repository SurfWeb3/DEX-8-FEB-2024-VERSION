...
/* Get the pending status from Redux state  */
...
/* "events[0]" gets the first event, which is the most recent one */
      ) : !isPending && events[0] ? (
        <div className="alert alert--remove" onClick={removeHandler} ref={alertRef}>
...
/* Use handler (removeHandler) & onClick to remove modal when not needed or wamted */
